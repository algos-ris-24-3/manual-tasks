...
## 4. Первый момент перераспределения

Нам нужно найти минимальное время, при котором происходит смена приоритетов (длительности становятся равными). Это будет первый момент, когда приоритеты двух задач совпадут, и потребуется перераспределение.

При поиске момента перераспределения достаточно рассмотреть только соседние по приоритету задачи, поскольку остатки несоседних задач не могут сравняться раньше. 
...
Минимальное значение t = 3 часа — это первый момент перераспределения, когда остатки задач A и B становятся равны, а также D и E.
...
Нам нужно найти минимальное время, при котором происходит смена приоритетов (длительности становятся равными).
...
Минимальное значение t = 2 часа — это момент перераспределения, когда все задачи достигают одинаковой длительности.
