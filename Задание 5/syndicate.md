# Вариант 3
Условие:

| Задания      |  A  |  B  |  C  |  D  |  E  |
|:-------------|:---:|:---:|:---:|:---:|:---:|
| Длительность |  18 |  18 |  12 |  12 |  6  |


| Исполнители           |  1  |  2  |  3  |
|:----------------------|:---:|:---:|:---:|
| Производительность(p) |  6  |  4  |  2  |

## Решение

1. Найдем длительность оптимального расписания по формуле
$$
    T_{min} = \frac {V_1 + V_2 + ... + V_n}{p_1 + p_2 + ... + p_k} = \frac {18+18+12+12+6}{6+4+2} = 3\frac {2}{3}
$$

### Первый период времени

2. Расставим приоритеты выполнения заданий

| Задания      |  A  |  B  |  C  |  D  |  E  |
|:-------------|:---:|:---:|:---:|:---:|:---:|
| Длительность |  18 |  18 |  12 |  12 |  6  |
| Приоритет    |  1  |  1  |  2  |  2  |  3  |

Так как задания A и B имеют одинаковый приоритет (1), **объединим их в AB.**
Так как задания C и D имеют одинаковый приоритет (2), **объединим их в CD.**

3. Назначим исполнителей на задание, при необходимости найдём их производительность.

На задание AB можно назначить максимум *двух* исполнителей, тогда:
$$
    P_{12} = \frac {8+6}{2} = 7
$$

На задание CD можно назначить максимум *одного* исполнителя, в этом случае, производительность $P_{3} = 4$.

4. Рассмотрим случаи равенства заданий для нахождения минимального времени


|    AB = CD   |  CD = E |
|:-------------|:-------:|
| 18-7t = 12-4t|12 - 4t = 6|
| 6 = 3t       |  4t = 6 |
| t = 2        |  t = 1,5|

Выбираем для периода **t = 1,5**

5. Тогда начало диаграммы Ганта будет выглядеть так:

```mermaid
gantt
    title Черновик диаграммы Ганта
    dateFormat DD HH:mm    
    axisFormat %H:%M
    Начало выполнения работ : milestone, m1, 01 00:00, 0h
    section Исполнитель 1
    A         :a1, 01 00:00, 0.75h
    B         :a2, after a1, 0.75h
    section Исполнитель 2
    B         :b1, 01 00:00, 0.75h
    A         :b2, after b1, 0.75h
    section Исполнитель 3
    C         :c1, 01 00:00, 0.75h
    D         :c2, after b1, 0.75h
    Окончание выполнения работ : milestone, m2, 01 03:40, 0h
```
### Второй период времени

6. Вычислим новые длительности заданий и обновим приоритеты

Для A и B: $18-7t = 18-7*1,5 = 7,5$

Для C и D: $12-4t = 12-4*1,5 = 6$

| Задания      |  A  |  B  |  C  |  D  |  E  |
|:-------------|:---:|:---:|:---:|:---:|:---:|
| Длительность |  7,5 |  7,5 |  6 |  6 |  6  |
| Приоритет    |  1  |  1  |  2  |  2  |  2  |

Так как задания A и B имеют одинаковый приоритет (1), **объединим их в AB.**
Так как задания C, D и E имеют одинаковый приоритет (2), **объединим их в CDE.**

7. Назначим исполнителей на задание, при необходимости найдём их производительность.

На задание AB можно назначить максимум *двух* исполнителей, тогда:
$$
    P_{12} = \frac {8+6}{2} = 7
$$

На задание CDE можно назначить максимум *одного* исполнителя, в этом случае, производительность $P_{3} = 4$.

8. Рассмотрим случай равенства заданий для нахождения минимального времени

$$
AB = CDE
$$
$$
7,5-7t = 6-4t
$$
$$
3t = 1,5
$$
$$
t = 0,5
$$

Выбираем для периода **t = 0,5**

9. Тогда начало диаграммы Ганта будет выглядеть так:

```mermaid
gantt
    title Черновик диаграммы Ганта
    dateFormat DD HH:mm    
    axisFormat %H:%M
    Начало выполнения работ : milestone, m1, 01 00:00, 0h
    section Исполнитель 1
    A         :a1, 01 00:00, 0.75h
    B         :a2, after a1, 0.75h
    A         :a3, 01 01:30, 0.25h
    B         :a4, after a3, 0.25h
    section Исполнитель 2
    B         :b1, 01 00:00, 0.75h
    A         :b2, after b1, 0.75h
    B         :b3, 01 01:30, 0.25h
    A         :b4, after b3, 0.25h
    section Исполнитель 3
    C         :c1, 01 00:00, 0.75h
    D         :c2, after c1, 0.75h
    C         :c3, 01 01:30, 0.167h
    D         :c4, after c3, 0.167h
    E         :c5, after c4, 0.167h
    Окончание выполнения работ : milestone, m2, 01 03:40, 0h
```

### Третий период времени

10. Вычислим новые длительности заданий и обновим приоритеты

Для A и B: $7,5-7*0,5 = 4$

Для C, D и E: $6-4*0,5 = 4$

| Задания      |  A  |  B  |  C  |  D  |  E  |
|:-------------|:---:|:---:|:---:|:---:|:---:|
| Длительность |  4 |  4 |  4 |  4 |  4  |
| Приоритет    |  1  |  1  |  1  |  1  |  1  |

Теперь все задания имеют одну длительность и одинаковый приоритет. Можем завершить построение диаграммы Ганта.


## Ответ 

**Длительность полученного расписание - $3\frac {2}{3}$**

```mermaid
gantt
    title Диаграмма оптимального расписания выполнения заданий
    dateFormat DD HH:mm    
    axisFormat %H:%M
    Начало выполнения работ : milestone, m1, 01 00:00, 0h
    section Исполнитель 1
    A         :a1, 01 00:00, 0.75h
    B         :a2, after a1, 0.75h
    A         :a3, 01 01:30, 0.25h
    B         :a4, after a3, 0.25h
    A         :a5, 01 02:00, 0.33h
    B         :a6, after a5, 0.33h
    C         :a7, after a6, 0.33h
    D         :a8, after a7, 0.33h
    E         :a9, after a8, 0.33h
    section Исполнитель 2
    B         :b1, 01 00:00, 0.75h
    A         :b2, after b1, 0.75h
    B         :b3, 01 01:30, 0.25h
    A         :b4, after b3, 0.25h
    B         :b5, 01 02:00, 0.33h
    C         :b6, after b5, 0.33h
    D         :b7, after b6, 0.33h
    E         :b8, after b7, 0.33h
    A         :b9, after b8, 0.33h
    section Исполнитель 3
    C         :c1, 01 00:00, 0.75h
    D         :c2, after c1, 0.75h
    C         :c3, 01 01:30, 0.167h
    D         :c4, after c3, 0.167h
    E         :c5, after c4, 0.167h
    C         :c5, 01 02:00, 0.33h
    D         :c6, after c5, 0.33h
    E         :c7, after c6, 0.33h
    A         :c8, after c7, 0.33h
    B         :c9, after c8, 0.33h
    Окончание выполнения работ : milestone, m2, 01 03:40, 0h
```